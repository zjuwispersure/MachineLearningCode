...
def create_data_model():
    ...
    data['demands'] = [0, 1, 1, 2]  # Demands at each location
    data['vehicle_capacities'] = [3, 3]  # Capacities of each vehicle
    data['num_vehicles'] = 2
    data['depot'] = 0  # Starting point for all vehicles
    return data
...
```
代码解释
创建数据模型：create_data_model 函数定义了距离矩阵、需求、车辆容量、车辆数量和仓库位置。
...
打印解决方案：print_solution 函数打印每辆车的路径、距离和负载。
通过运行这个代码，你可以得到一个满足容量限制的车辆路径方案。这个示例展示了如何使用 OR-Tools 来解决带有容量限制的车辆路径问题。
